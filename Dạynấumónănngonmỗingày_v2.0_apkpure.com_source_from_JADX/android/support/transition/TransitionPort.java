...
abstract class TransitionPort implements Cloneable {
    ...
    protected void createAnimators(ViewGroup sceneRoot, TransitionValuesMaps startValues, TransitionValuesMaps endValues) {
        ...
        i = 0;
        while (true) {
            if (i >= endValues.itemIdValues.size()) {
                break;
            }
        ...
        for (View view : startValues.viewValues.keySet()) {
            ...
            if (isInListView) {
                ...
                id = view2.getId();
                if (startValues.viewValues.get(view2) != null) {
                    start = (TransitionValues) startValues.viewValues.get(view2);
                } else {
                    start = (TransitionValues) startValues.idValues.get(id);
                }
        ...
        for (View view22 : endCopy.keySet()) {
            ...
            if (isValidTarget(view22, (long) id)) {
                if (startValues.viewValues.get(view22) != null) {
                    start = (TransitionValues) startValues.viewValues.get(view22);
                } else {
                    start = (TransitionValues) startValues.idValues.get(id);
                }
    ...
    private void runAnimator(Animator animator, ArrayMap<Animator, AnimationInfo> runningAnimators) {
        if (animator != null) {
            animator.addListener(new C00521(runningAnimators));
            animate(animator);
        }
    }
    ...
    protected void animate(Animator animator) {
        ...
        animator.addListener(new C00532());
        animator.start();
    }
