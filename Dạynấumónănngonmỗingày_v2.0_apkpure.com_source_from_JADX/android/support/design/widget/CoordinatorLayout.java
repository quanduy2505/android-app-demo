...
public class CoordinatorLayout extends ViewGroup implements NestedScrollingParent {
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        LayoutParams(Context context, AttributeSet attrs) {
            ...
            this.dodgeInsetEdges = CoordinatorLayout.TYPE_ON_INTERCEPT;
            this.mLastChildRect = new Rect();
            TypedArray a = context.obtainStyledAttributes(attrs, C0023R.styleable.CoordinatorLayout_Layout);
            this.gravity = a.getInteger(C0023R.styleable.CoordinatorLayout_Layout_android_layout_gravity, CoordinatorLayout.TYPE_ON_INTERCEPT);
            this.mAnchorId = a.getResourceId(C0023R.styleable.CoordinatorLayout_Layout_layout_anchor, -1);
            this.anchorGravity = a.getInteger(C0023R.styleable.CoordinatorLayout_Layout_layout_anchorGravity, CoordinatorLayout.TYPE_ON_INTERCEPT);
            this.keyline = a.getInteger(C0023R.styleable.CoordinatorLayout_Layout_layout_keyline, -1);
            this.insetEdge = a.getInt(C0023R.styleable.CoordinatorLayout_Layout_layout_insetEdge, CoordinatorLayout.TYPE_ON_INTERCEPT);
            this.dodgeInsetEdges = a.getInt(C0023R.styleable.CoordinatorLayout_Layout_layout_dodgeInsetEdges, CoordinatorLayout.TYPE_ON_INTERCEPT);
            this.mBehaviorResolved = a.hasValue(C0023R.styleable.CoordinatorLayout_Layout_layout_behavior);
            if (this.mBehaviorResolved) {
                this.mBehavior = CoordinatorLayout.parseBehavior(context, attrs, a.getString(C0023R.styleable.CoordinatorLayout_Layout_layout_behavior));
            }
            a.recycle();
    ...
    class C08471 implements OnApplyWindowInsetsListener {
        C08471() {
        }
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        static class C08481 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C08481() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C08481());
        }
    }
    ...
    public CoordinatorLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0023R.styleable.CoordinatorLayout, defStyleAttr, C0023R.style.Widget_Design_CoordinatorLayout);
        int keylineArrayRes = a.getResourceId(C0023R.styleable.CoordinatorLayout_keylines, TYPE_ON_INTERCEPT);
        ...
        this.mStatusBarBackground = a.getDrawable(C0023R.styleable.CoordinatorLayout_statusBarBackground);
        a.recycle();
        setupForInsets();
    ...
    static Behavior parseBehavior(Context context, AttributeSet attrs, String name) {
        ...
        try {
            ...
            objArr[TYPE_ON_TOUCH] = attrs;
            return (Behavior) c.newInstance(objArr);
        } catch (Exception e) {
    ...
    private void prepareChildren() {
        ...
        for (int i = TYPE_ON_INTERCEPT; i < count; i += TYPE_ON_TOUCH) {
            ...
            for (int j = TYPE_ON_INTERCEPT; j < count; j += TYPE_ON_TOUCH) {
                if (j != i) {
    ...
    private void getDesiredAnchoredChildRectWithoutConstraints(View child, int layoutDirection, Rect anchorRect, Rect out, LayoutParams lp, int childWidth, int childHeight) {
        ...
        int vgrav = absGravity & C0801R.styleable.AppCompatTheme_spinnerStyle;
        int anchorVgrav = absAnchorGravity & C0801R.styleable.AppCompatTheme_spinnerStyle;
        switch (absAnchorGravity & 7) {
            case TYPE_ON_TOUCH /*1*/:
                left = anchorRect.left + (anchorRect.width() / EVENT_VIEW_REMOVED);
                break;
        ...
        switch (anchorVgrav) {
            case ConnectionResult.API_UNAVAILABLE /*16*/:
                top = anchorRect.top + (anchorRect.height() / EVENT_VIEW_REMOVED);
                break;
        ...
        switch (hgrav) {
            case TYPE_ON_TOUCH /*1*/:
                left -= childWidth / EVENT_VIEW_REMOVED;
                break;
            case ConnectionResult.INVALID_ACCOUNT /*5*/:
                break;
            default:
        ...
        switch (vgrav) {
            case ConnectionResult.API_UNAVAILABLE /*16*/:
                top -= childHeight / EVENT_VIEW_REMOVED;
                break;
            case C0801R.styleable.AppCompatTheme_panelMenuListWidth /*80*/:
                break;
            default:
    ...
    private void layoutChildWithKeyline(View child, int keyline, int layoutDirection) {
        ...
        switch (vgrav) {
            case ConnectionResult.API_UNAVAILABLE /*16*/:
                top = TYPE_ON_INTERCEPT + (childHeight / EVENT_VIEW_REMOVED);
                break;
            case C0801R.styleable.AppCompatTheme_panelMenuListWidth /*80*/:
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (lp.mBehavior != null) {
            ...
            if (scrimAlpha > 0.0f) {
                ...
                this.mScrimPaint.setAlpha(MathUtils.constrain(Math.round(255.0f * scrimAlpha), (int) TYPE_ON_INTERCEPT, (int) MotionEventCompat.ACTION_MASK));
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i += TYPE_ON_TOUCH) {
            ...
            if (type != 0 || child.getVisibility() != 8) {
                ...
                if (!(lp.insetEdge == 0 || drawRect.isEmpty())) {
                    ...
                    switch (absInsetEdge & C0801R.styleable.AppCompatTheme_spinnerStyle) {
    ...
    public void dispatchDependentViewsChanged(View view) {
        ...
        if (dependents != null && !dependents.isEmpty()) {
            for (int i = TYPE_ON_INTERCEPT; i < dependents.size(); i += TYPE_ON_TOUCH) {
            ...
            if (view.getVisibility() != 8) {
                ...
                if (lp.isNestedScrollAccepted()) {
                    ...
                    if (viewBehavior != null) {
                        ...
                        if (dx > 0) {
                            xConsumed = Math.max(xConsumed, this.mTempIntPair[TYPE_ON_INTERCEPT]);
                        } else {
                            xConsumed = Math.min(xConsumed, this.mTempIntPair[TYPE_ON_INTERCEPT]);
                        }
                        if (dy > 0) {
                            yConsumed = Math.max(yConsumed, this.mTempIntPair[TYPE_ON_TOUCH]);
                        } else {
                            yConsumed = Math.min(yConsumed, this.mTempIntPair[TYPE_ON_TOUCH]);
                        }
                        accepted = true;
                    }
                }
        ...
        if (VERSION.SDK_INT >= 21) {
            if (ViewCompat.getFitsSystemWindows(this)) {
                if (this.mApplyWindowInsetsListener == null) {
                    this.mApplyWindowInsetsListener = new C08471();
                }
