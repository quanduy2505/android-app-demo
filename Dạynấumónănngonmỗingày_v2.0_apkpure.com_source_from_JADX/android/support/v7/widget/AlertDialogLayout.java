    ...
    private boolean tryOnMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != 8) {
                int id = child.getId();
                if (id == C0181R.id.topPanel) {
                    topPanel = child;
                } else if (id == C0181R.id.buttonPanel) {
                    buttonPanel = child;
                } else if (id != C0181R.id.contentPanel && id != C0181R.id.customPanel) {
                    return false;
                } else {
                    if (middlePanel != null) {
                        return false;
            ...
            if (!(child == null || child.getVisibility() == 8)) {
                ...
                switch (GravityCompat.getAbsoluteGravity(layoutGravity, ViewCompat.getLayoutDirection(this)) & 7) {
                    case OnSubscribeConcatMap.BOUNDARY /*1*/:
                        ...
                        childLeft = ((((childSpace - childWidth) / 2) + paddingLeft) + r0) - lp.rightMargin;
                        break;
