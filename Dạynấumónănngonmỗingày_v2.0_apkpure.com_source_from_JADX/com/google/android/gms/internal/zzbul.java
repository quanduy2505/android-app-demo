...
public final class zzbul {
    ...
    private zzbul(byte[] bArr, int i, int i2) {
        this.zzcrS = UrlImageViewHelper.CACHE_DURATION_INFINITE;
        this.zzcrU = 64;
        this.zzcrV = 67108864;
        this.buffer = bArr;
        this.zzcrN = i;
    ...
    public int zzacD() throws IOException {
        ...
        int i = zzacK & TransportMediator.KEYCODE_MEDIA_PAUSE;
        byte zzacK2 = zzacK();
        if (zzacK2 >= null) {
            return i | (zzacK2 << 7);
        }
        i |= (zzacK2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 7;
        zzacK2 = zzacK();
        if (zzacK2 >= null) {
            return i | (zzacK2 << 14);
        }
        i |= (zzacK2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 14;
        zzacK2 = zzacK();
        if (zzacK2 >= null) {
            return i | (zzacK2 << 21);
        }
        i |= (zzacK2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 21;
        zzacK2 = zzacK();
        i |= zzacK2 << 28;
        if (zzacK2 >= null) {
            return i;
    ...
    public long zzacE() throws IOException {
        ...
        for (int i = 0; i < 64; i += 7) {
            byte zzacK = zzacK();
            j |= ((long) (zzacK & TransportMediator.KEYCODE_MEDIA_PAUSE)) << i;
            if ((zzacK & TransportMediator.FLAG_KEY_MEDIA_NEXT) == 0) {
                return j;
            }
        }
        throw zzbus.zzacT();
    ...
    public int zzacF() throws IOException {
        return (((zzacK() & MotionEventCompat.ACTION_MASK) | ((zzacK() & MotionEventCompat.ACTION_MASK) << 8)) | ((zzacK() & MotionEventCompat.ACTION_MASK) << 16)) | ((zzacK() & MotionEventCompat.ACTION_MASK) << 24);
    }
    ...
    public int zzacI() {
        if (this.zzcrS == UrlImageViewHelper.CACHE_DURATION_INFINITE) {
            return -1;
        }
