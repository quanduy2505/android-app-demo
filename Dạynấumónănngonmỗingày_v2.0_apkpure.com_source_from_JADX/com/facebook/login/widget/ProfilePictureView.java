...
public class ProfilePictureView extends FrameLayout {
    ...
    class C10151 implements Callback {
        C10151() {
        }
    ...
    private void parseAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, C0378R.styleable.com_facebook_profile_picture_view);
        setPresetSize(a.getInt(C0378R.styleable.com_facebook_profile_picture_view_com_facebook_preset_size, CUSTOM));
        this.isCropped = a.getBoolean(C0378R.styleable.com_facebook_profile_picture_view_com_facebook_is_cropped, IS_CROPPED_DEFAULT_VALUE);
        a.recycle();
    }
    ...
    private void setBlankProfilePicture() {
        ...
        if (this.customizedDefaultProfilePicture == null) {
            setImageBitmap(BitmapFactory.decodeResource(getResources(), isCropped() ? C0378R.drawable.com_facebook_profile_picture_blank_square : C0378R.drawable.com_facebook_profile_picture_blank_portrait));
            return;
        }
    ...
    private void sendImageRequest(boolean allowCachedResponse) {
        ImageRequest request = new Builder(getContext(), ImageRequest.getProfilePictureUri(this.profileId, this.queryWidth, this.queryHeight)).setAllowCachedRedirects(allowCachedResponse).setCallerTag(this).setCallback(new C10151()).build();
        if (this.lastRequest != null) {
            ImageDownloader.cancelRequest(this.lastRequest);
        }
    ...
    private int getPresetSizeInPixels(boolean forcePreset) {
        ...
        switch (this.presetSizeType) {
            case LARGE /*-4*/:
                dimensionId = C0378R.dimen.com_facebook_profilepictureview_preset_size_large;
                break;
            case NORMAL /*-3*/:
                dimensionId = C0378R.dimen.com_facebook_profilepictureview_preset_size_normal;
                break;
            case SMALL /*-2*/:
                dimensionId = C0378R.dimen.com_facebook_profilepictureview_preset_size_small;
                break;
            case CUSTOM /*-1*/:
                if (forcePreset) {
                    dimensionId = C0378R.dimen.com_facebook_profilepictureview_preset_size_normal;
