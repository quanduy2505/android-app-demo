...
public class LoginButton extends FacebookButtonBase {
    ...
    class C04441 implements Runnable {
        ...
        class C04431 implements Runnable {
            ...
            C04431(FetchedAppSettings fetchedAppSettings) {
        ...
        public void run() {
            LoginButton.this.getActivity().runOnUiThread(new C04431(FetchedAppSettingsManager.queryAppSettings(this.val$appId, false)));
        }
    }

    /* renamed from: com.facebook.login.widget.LoginButton.3 */
    static /* synthetic */ class C04453 {
    ...
    protected class LoginClickListener implements OnClickListener {
        ...
        class C04461 implements DialogInterface.OnClickListener {
            final /* synthetic */ LoginManager val$loginManager;

            C04461(LoginManager loginManager) {
        ...
        protected void performLogout(Context context) {
            ...
            if (LoginButton.this.confirmLogout) {
                String message;
                String logout = LoginButton.this.getResources().getString(C0378R.string.com_facebook_loginview_log_out_action);
                String cancel = LoginButton.this.getResources().getString(C0378R.string.com_facebook_loginview_cancel_action);
                Profile profile = Profile.getCurrentProfile();
                if (profile == null || profile.getName() == null) {
                    message = LoginButton.this.getResources().getString(C0378R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    message = String.format(LoginButton.this.getResources().getString(C0378R.string.com_facebook_loginview_logged_in_as), new Object[]{profile.getName()});
                }
                Builder builder = new Builder(context);
                builder.setMessage(message).setCancelable(true).setPositiveButton(logout, new C04461(loginManager)).setNegativeButton(cancel, null);
                builder.create().show();
                return;
            }
            loginManager.logOut();
    ...
    class C10142 extends AccessTokenTracker {
        C10142() {
        }
    ...
    private void checkToolTipSettings() {
        switch (C04453.$SwitchMap$com$facebook$login$widget$LoginButton$ToolTipMode[this.toolTipMode.ordinal()]) {
            case OnSubscribeConcatMap.BOUNDARY /*1*/:
                FacebookSdk.getExecutor().execute(new C04441(Utility.getMetadataApplicationId(getContext())));
            case OnSubscribeConcatMap.END /*2*/:
                displayToolTip(getResources().getString(C0378R.string.com_facebook_tooltip_default));
            default:
        }
    }
    ...
    protected void configureButton(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C0378R.color.com_facebook_blue));
            this.loginText = "Continue with Facebook";
        } else {
            this.accessTokenTracker = new C10142();
        }
        setButtonText();
        setCompoundDrawablesWithIntrinsicBounds(AppCompatResources.getDrawable(getContext(), C0378R.drawable.com_facebook_button_login_logo), null, null, null);
    }
    ...
    protected int getDefaultStyleResource() {
        return C0378R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C0378R.styleable.com_facebook_login_view, defStyleAttr, defStyleRes);
        try {
            this.confirmLogout = a.getBoolean(C0378R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = a.getString(C0378R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = a.getString(C0378R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(a.getInt(C0378R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (text == null) {
            text = resources.getString(C0378R.string.com_facebook_loginview_log_in_button_continue);
            logInWidth = measureButtonWidth(text);
            if (resolveSize(logInWidth, widthMeasureSpec) < logInWidth) {
                text = resources.getString(C0378R.string.com_facebook_loginview_log_in_button);
            }
        }
        logInWidth = measureButtonWidth(text);
        text = this.logoutText;
        if (text == null) {
            text = resources.getString(C0378R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            ...
            if (this.logoutText != null) {
                ...
                charSequence = resources.getString(C0378R.string.com_facebook_loginview_log_out_button);
            }
            setText(charSequence);
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            String text = resources.getString(C0378R.string.com_facebook_loginview_log_in_button_continue);
            int width = getWidth();
