...
public final class OperatorPublish<T> extends ConnectableObservable<T> {
    ...
    static final class PublishSubscriber<T> extends Subscriber<T> implements Subscription {
        ...
        void init() {
            add(Subscriptions.create(new C15331()));
        }
    ...
    static class C16091 implements OnSubscribe<T> {
        final /* synthetic */ AtomicReference val$curr;

        C16091(AtomicReference atomicReference) {
            this.val$curr = atomicReference;
        }

        public void call(Subscriber<? super T> child) {
            while (true) {
                PublishSubscriber<T> r = (PublishSubscriber) this.val$curr.get();
                if (r == null || r.isUnsubscribed()) {
                    PublishSubscriber<T> u = new PublishSubscriber(this.val$curr);
                    u.init();
                    if (this.val$curr.compareAndSet(r, u)) {
                        r = u;
                    } else {
                        continue;
                    }
    ...
    static class C16102 implements OnSubscribe<R> {
        ...
        class C14121 extends Subscriber<R> {
            final /* synthetic */ Subscriber val$child;
            final /* synthetic */ OnSubscribePublishMulticast val$op;

            C14121(Subscriber subscriber, OnSubscribePublishMulticast onSubscribePublishMulticast) {
            ...
            public void setProducer(Producer p) {
                this.val$child.setProducer(p);
            }
        }

        C16102(boolean z, Func1 func1, Observable observable) {
        ...
        public void call(Subscriber<? super R> child) {
            OnSubscribe op = new OnSubscribePublishMulticast(RxRingBuffer.SIZE, this.val$delayError);
            Subscriber<R> subscriber = new C14121(child, op);
            child.add(op);
            child.add(subscriber);
            ((Observable) this.val$selector.call(Observable.create(op))).unsafeSubscribe(subscriber);
            this.val$source.unsafeSubscribe(op.subscriber());
        }
    }

    public static <T> ConnectableObservable<T> create(Observable<? extends T> source) {
        AtomicReference<PublishSubscriber<T>> curr = new AtomicReference();
        return new OperatorPublish(new C16091(curr), source, curr);
    }
    ...
    public static <T, R> Observable<R> create(Observable<? extends T> source, Func1<? super Observable<T>, ? extends Observable<R>> selector, boolean delayError) {
        return Observable.create(new C16102(delayError, selector, source));
    }
