...
public final class OperatorWithLatestFrom<T, U, R> implements Operator<R, T> {
    ...
    class C14541 extends Subscriber<T> {
        final /* synthetic */ AtomicReference val$current;
        final /* synthetic */ SerializedSubscriber val$s;

        C14541(Subscriber x0, boolean x1, AtomicReference atomicReference, SerializedSubscriber serializedSubscriber) {
    ...
    class C14552 extends Subscriber<U> {
        final /* synthetic */ AtomicReference val$current;
        final /* synthetic */ SerializedSubscriber val$s;

        C14552(AtomicReference atomicReference, SerializedSubscriber serializedSubscriber) {
            this.val$current = atomicReference;
            this.val$s = serializedSubscriber;
        }
    ...
    public Subscriber<? super T> call(Subscriber<? super R> child) {
        ...
        Subscriber<T> subscriber = new C14541(s, true, current, s);
        Subscriber<U> otherSubscriber = new C14552(current, s);
        s.add(subscriber);
