...
public final class OnSubscribeAmb<T> implements OnSubscribe<T> {
    ...
    class C12592 implements Producer {
        final /* synthetic */ AtomicReference val$choice;
        final /* synthetic */ Selection val$selection;

        C12592(AtomicReference atomicReference, Selection selection) {
            this.val$choice = atomicReference;
            this.val$selection = selection;
        }

        public void request(long n) {
            AmbSubscriber<T> c = (AmbSubscriber) this.val$choice.get();
            if (c != null) {
                c.requestMore(n);
                return;
            }
            for (AmbSubscriber<T> ambSubscriber : this.val$selection.ambSubscribers) {
                if (!ambSubscriber.isUnsubscribed()) {
                    if (this.val$choice.get() == ambSubscriber) {
    ...
    class C15131 implements Action0 {
        final /* synthetic */ AtomicReference val$choice;
        final /* synthetic */ Selection val$selection;

        C15131(AtomicReference atomicReference, Selection selection) {
            this.val$choice = atomicReference;
            this.val$selection = selection;
        }
    ...
    public void call(Subscriber<? super T> subscriber) {
        ...
        AtomicReference<AmbSubscriber<T>> choice = selection.choice;
        subscriber.add(Subscriptions.create(new C15131(choice, selection)));
        ...
        subscriber.setProducer(new C12592(choice, selection));
    }
