...
public final class OperatorTakeUntil<T, E> implements Operator<T, T> {
    ...
    class C14371 extends Subscriber<T> {
        final /* synthetic */ Subscriber val$serial;

        C14371(Subscriber x0, boolean x1, Subscriber subscriber) {
    ...
    class C14382 extends Subscriber<E> {
        final /* synthetic */ Subscriber val$main;

        C14382(Subscriber subscriber) {
    ...
    public Subscriber<? super T> call(Subscriber<? super T> child) {
        ...
        Subscriber<T> main = new C14371(serial, false, serial);
        Subscriber<E> so = new C14382(main);
        serial.add(main);
        serial.add(so);
        child.add(serial);
