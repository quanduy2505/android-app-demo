...
public final class SingleDelayedProducer<T> extends AtomicInteger implements Producer {
    ...
    public void request(long n) {
        if (n < 0) {
            ...
            if (s == NO_REQUEST_HAS_VALUE && compareAndSet(NO_REQUEST_HAS_VALUE, HAS_REQUEST_HAS_VALUE)) {
    ...
    public void setValue(T value) {
        do {
            ...
            if (s == 0) {
                this.value = value;
            } else if (s == HAS_REQUEST_NO_VALUE && compareAndSet(HAS_REQUEST_NO_VALUE, HAS_REQUEST_HAS_VALUE)) {
