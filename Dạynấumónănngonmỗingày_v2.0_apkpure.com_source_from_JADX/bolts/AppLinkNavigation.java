...
public class AppLinkNavigation {
    ...
    /* renamed from: bolts.AppLinkNavigation.1 */
    static class C09411 implements Continuation<AppLink, NavigationResult> {
        ...
        public NavigationResult then(Task<AppLink> task) throws Exception {
            return AppLinkNavigation.navigate(this.val$context, (AppLink) task.getResult());
        }
    }
    ...
    public NavigationResult navigate(Context context) {
        ...
        for (Target target : getAppLink().getTargets()) {
            ...
            if (pm.resolveActivity(targetIntent, NativeProtocol.MESSAGE_GET_ACCESS_TOKEN_REQUEST) != null) {
    ...
    public static Task<NavigationResult> navigateInBackground(Context context, Uri destination, AppLinkResolver resolver) {
        return resolver.getAppLinkFromUrlInBackground(destination).onSuccess(new C09411(context), Task.UI_THREAD_EXECUTOR);
    }
